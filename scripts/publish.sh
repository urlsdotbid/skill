...
BASE_URL="https://api.urls.bid"
CREDENTIALS_FILE="$HOME/.urlsbid/credentials"
API_KEY="${URLSBID_API_KEY:-}"
API_KEY_SOURCE="none"
if [[ -n "${URLSBID_API_KEY:-}" ]]; then
  API_KEY_SOURCE="env"
fi
ALLOW_NON_URLSBID_BASE_URL=0
SLUG=""
CLAIM_TOKEN=""
TITLE=""
DESCRIPTION=""
...
Options:
  --api-key <key>         API key (or set $URLSBID_API_KEY)
  ...
  --base-url <url>        API base (default: https://api.urls.bid)
  --allow-nonurlsbid-base-url
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --allow-nonurlsbid-base-url) ALLOW_NON_URLSBID_BASE_URL=1; shift ;;
...
# Auto-sync root script with web public skill script and npm skill script if we are in local development monorepo
DEV_SKILL_SCRIPT="$SCRIPT_DIR/../apps/web/public/skills/urls-bid/scripts/publish.sh"
NPM_SKILL_DIR="$SCRIPT_DIR/../scratch-deploy/skill"

if [[ -f "$DEV_SKILL_SCRIPT" ]]; then
  if ! cmp -s "$BASH_SOURCE" "$DEV_SKILL_SCRIPT"; then
    echo "Syncing local publish.sh changes with web public..." >&2
    if [[ "$BASH_SOURCE" -nt "$DEV_SKILL_SCRIPT" ]]; then
      cp "$BASH_SOURCE" "$DEV_SKILL_SCRIPT"
    else
      cp "$DEV_SKILL_SCRIPT" "$BASH_SOURCE"
      exec "$BASH_SOURCE" "$@"
    fi
  fi
fi

if [[ -d "$NPM_SKILL_DIR" ]]; then
  NPM_SKILL_SCRIPT="$NPM_SKILL_DIR/scripts/publish.sh"
  mkdir -p "$(dirname "$NPM_SKILL_SCRIPT")"
  if ! cmp -s "$BASH_SOURCE" "$NPM_SKILL_SCRIPT" 2>/dev/null; then
    echo "Syncing local publish.sh changes with npm skill..." >&2
    cp "$BASH_SOURCE" "$NPM_SKILL_SCRIPT"
    chmod +x "$NPM_SKILL_SCRIPT"
  fi
fi
...
# Safety guard
if [[ -n "$API_KEY" && "$BASE_URL" != "https://api.urls.bid" && "$ALLOW_NON_URLSBID_BASE_URL" -ne 1 ]]; then
  die "refusing to send API key to non-default base URL; pass --allow-nonurlsbid-base-url to override"
fi
...
if [[ -f "$TARGET" ]]; then
  ...
  while IFS= read -r -d '' f; do
    ...
    [[ "$rel" == ".urlsbid/fork-meta.json" ]] && continue
    sz=$(wc -c < "$f" | tr -d ' ')
...
CLIENT_HEADER_VALUE="urls-bid-publish-sh"
if [[ -n "$CLIENT" ]]; then
...
CLIENT_ARGS=(-H "x-urlsbid-client: $CLIENT_HEADER_VALUE")
